...
Status snapshot for picking this work back up. **Last updated 2026-09-20** (latest: **2026-09-20 Chart key shows Actual as green and red** at the bottom; before that: **2026-09-20 Copy pass across the app** at the bottom; before that: **2026-09-20 "Update balance" vs "Refresh" wording** at the bottom; before that: **2026-09-20 Account line restored in iOS tracker header** at the bottom; before that: **2026-09-20 Direction explanation on Add Tracker** at the bottom; before that: **2026-09-20 Tracker budget wording; Add Tracker footers removed** at the bottom; before that: **2026-09-20 Four more tracker colours** at the bottom; before that: **2026-09-19 Wobbly design refresh across every surface** at the bottom; before that: **2026-09-19 Hand-wobbled chart lines** at the bottom; before that **2026-09-19 Loading/empty mark: stronger wobble**; before that **2026-09-19 Live rings drawn hand-wobbled everywhere**; before that **2026-09-19 Empty states and placeholders match the new icon**; before that **2026-09-19 App icon redesign — wobbly two rings**; before that **2026-09-19 Docs sync** at the bottom; before that **2026-09-19 macOS: sheet titles flush-left**, at the bottom; before that **2026-09-19 macOS: menu bar dropdown trimmed, New Tracker name-field suggestions off**, at the bottom; before that **2026-09-19 watch rings: number and colour only**),
...
## 2026-09-19 App icon redesign — wobbly two rings

The icon (a cream sine wave on violet) had nothing to do with the app's UI, so it was re-thought in a brainstorm-with-mockups session and replaced. Final design: two hand-wobbled, partly closed rings — outer pace ring lavender ~75% closed, inner actual ring in the app's `good` green ~68% closed (slightly ahead of pace; the gap is the "wiggle room"). Decisions along the way: the rings came back, but drawn as irregular hand-drawn outlines with open gaps and only two rings so they don't read as Apple Fitness (a stated worry); wobble level "more wobbly" was picked over subtler (invisible at small sizes) and stronger (blobby, uneven gap); outer ring was pulled back from 85% to 75%. Light icon: violet `#7062D8` (lighter than in-app `brand` `#594BBD`, deliberately), lavender `#DAD5F7`, green `#52C799` (the `good` dark-mode value, brighter on violet). Dark icon: `#0F0C2A`, lavender `#9E8FFA`, same green. Tinted: grayscale.

Everything is generated by the new [`scripts/generate_app_icon.py`](scripts/generate_app_icon.py) (Pillow; one geometry definition, 4x supersampled) — it rewrites the iOS light/dark/tinted 1024s, all ten macOS sizes (light version, full-bleed as before), the watchOS 1024 (light), and the tinted `WiggleRoomMark` the widgets use as their corner brand mark. No Swift or `Contents.json` changes were needed (same filenames). The AppIcon is iOS-only-styled per the earlier design pass; spec §3.3 and §12 were rewritten to describe the new icon.

Verified: rendered the three iOS variants and looked at them; `xcrun actool` compiles both asset catalogs (iOS AppIcon produced). Not verified: a full `xcodebuild`, how the icon looks on a real Home Screen/Dock/watch, the tinted variant under a real iOS tint, or the widget corner mark at its final size. The 16/32px macOS renders are plain downscales — the wobble is essentially gone there (two clean arcs), which is expected.

## 2026-09-19 Empty states and placeholders match the new icon

Audit of every static/placeholder asset after the icon change. Only three asset catalogs exist (app, watch, shared): `AccentColor` is the unchanged `brand` violet, the AppIcons and `WiggleRoomMark` were regenerated last entry, and there is no launch-screen image (generated, plain). The rest were code-drawn placeholders, updated in [EmptyRingsMark.swift](src/WiggleRoomShared/EmptyRingsMark.swift): `EmptyRingsMark` is now built from a new `IconRingShape` (the icon's exact wobbly outlines, trimmed to ~75% outer / ~68% inner) instead of two perfect concentric circles in violet/coral; the inner ring is faint `good` green like the icon. That one change updates the iOS empty tracker list, `WiggleEmptyState` (now `EmptyRingsMark(size: 120)` instead of a plain circle), and the widgets' loading states. Also moved onto it: macOS's two empty panes (were stock `ContentUnavailableView` + `circle.circle`; "No Trackers Yet" now has an "Add a Tracker" button like iOS), the watch's empty list, the menu bar dropdown's "No trackers yet", the single-tracker widget's "No Tracker" state, and the complication's empty state (were `circle.dashed`). Left alone deliberately: Live Activity `circle.circle` symbols, the Control's SF Symbol, and the "Tracker Deleted" `ContentUnavailableView` (system glyphs, not brand placeholders).
...
## 2026-09-19 Live rings drawn hand-wobbled everywhere

The real rings now use the app icon's wobbly outlines instead of perfect circles. One change does most of it: `RingsView.ring(...)` ([RingsView.swift](src/WiggleRoomShared/RingsView.swift)) draws `IconRingShape` (in [EmptyRingsMark.swift](src/WiggleRoomShared/EmptyRingsMark.swift), new `fitsRect` option that stretches the outline to fill its frame like a `Circle`) — so the dashboard, list rows, macOS sidebar, menu bar dropdown, watch detail, home-screen/lock-screen/complication rectangular rings and every widget size follow automatically, with the same `trim`-based animation. Fill is by arc length so the fraction stays exact; the shape starts at 12 o'clock going clockwise, so the old `-90°` rotation was removed. Also converted: the two dashed "Add Tracker" rings (iOS list, macOS sidebar), the Celebration disc (now a wobbly blob), and the two system `Gauge(.accessoryCircular)` uses (watch complication, Lock Screen circular) — replaced by new [WobblyGauge.swift](src/WiggleRoomShared/WobblyGauge.swift) since the system arc can't be restyled. Not changed: the widget corner brand mark (circle-clipped icon image), SF Symbol glyphs, the trend chart.

Tradeoff/decision: the inner/outer ring gap went from `0.12` to `0.25 × lineWidth` — with two different outlines the old sliver risked the bands touching. Checked numerically (Python, sampling both outlines at L/S = 0.077–0.17) that they stay clear at either gap; 0.25 also lets the wobble read. This moves the rings further from Fitness's tight look, which was the intent.

Also: the macOS build failed once with an unrelated-looking `ValueSnapshot`/`Identifiable` error when it shared a DerivedData folder with the iOS build; a clean macOS-only DerivedData built fine — an artefact of sharing one folder, not the code.

Verified: iOS, macOS, watchOS, Widgets and Complication schemes build. Not verified — nothing was run or screenshotted (the watch app wouldn't launch in the simulator from here, and there's no tracker data on it anyway): the wobble against the centre text at each size, the ring-to-ring gap visually, `WobblyGauge` under Lock Screen vibrant / watch tinted rendering (it draws in `.primary`/status colour, not the system gauge's accent), and the 4–6pt list/row rings where the wobble is barely visible. Check the dashboard, a widget and the Lock Screen circular first.

## 2026-09-19 Loading/empty mark: stronger wobble

Reported: the widgets' "Loading data…" placeholder didn't look wobbly. It is `EmptyRingsMark` at ~56pt, faint — the icon's own outline (a few percent radius deviation) is invisible at that size and opacity. `IconRingShape` gained a `pronounced` option (the earlier "most wobbly" outlines from the icon exploration), and `EmptyRingsMark` now uses it with both rings fitted to their frames (inner inset 17% of size), stroke unchanged at 9% of size, opacity raised (outer 0.35→0.5, inner 0.55→0.7). This applies to every use of the mark (empty tracker list, empty states, widget/complication/watch/menu bar empty and loading states). Widgets and macOS schemes build; not run or looked at — check the widget loading state and the empty tracker list first, and say if it's now too lumpy.

## 2026-09-19 Hand-wobbled chart lines

Reported: the charts looked too clinical next to the wobbly rings. [TrendChartView.swift](src/WiggleRoomShared/TrendChartView.swift): the Budget line and each actual-reading segment used to be two-point straight `LineMark`s; they are now `wobbly(from:to:seed:)` — 9 points along the true line with a deterministic sideways offset (`amplitude × sin(πt) × sin(2π·cycles·t + seed)`, amplitude ~1.8% of the Y range) drawn with `.catmullRom`. The `sin(πt)` envelope makes the offset exactly zero at both ends, so start/end values and the connections between readings are exact, and the `PointMark`s sit on true values; only the strokes in between wander (per-segment seeds differ so lines don't all wiggle in unison). Values are clamped into `yDomain`, keeping the earlier Charts rendering-bug guard (§3.5 / trend clamp) intact. Gridlines (both axes) became thin dashed strokes to soften the grid. Trend line untouched (already curved, dotted). One deliberate loosening: the Budget line was described as "the straight, fixed yardstick" — it now wanders up to ~1.8% of the visible range off the true line mid-period, so it's a look, not a precise reference; spec §3.5 updated. iOS, macOS, watchOS and Widgets build; not run or looked at — check a real chart first (dashboard, then the watch's 130pt one and the chart widget, where the fixed ~1.8% may read as too much or too little), and a short segment between two close readings.


## 2026-09-19 Chart key removed on watch and widgets

The Budget/Actual/Trend legend was too large on the watch and isn't needed on widgets. `TrendChartView` now gets `showsLegend: false` in [WatchTrackerDetailView.swift](src/WiggleRoomWatch/WatchTrackerDetailView.swift) and the extra-large Trend Chart widget (the other widget sizes already had it off). iOS/macOS detail keeps the key. Watch and Widgets schemes build; not run.

## 2026-09-19 Wobbly design refresh across every surface

Implemented the approved mock-up direction (spec §3.2b). **New:** `Tracker.colorIndex`/`glyph` (synced), `TrackerPalette`, `WobblyStyle` (`BlobShape`, `WobblyCard`, `WobblyBadge`/`TrackerBadge`, `TrackerWidgetBackground`), `TrackerAppearancePicker`, `SettingsLabel`/`ProviderBadge`, bundled **Nunito** (`Font.wiggleText`, applied as the app-wide default; ~95 `.font` call sites converted). **Restyled:** iOS list rows/detail/cards/chart legend/celebration/log/history/settings/source screens, Mac sidebar + menu bar + settings, watch list/detail (custom Fraunces headings, card list), every widget family + Live Activity (`TrackerActivityAttributes` gained optional `colorIndex`/`glyph`) + Control glyph + complication. No behaviour changed.

Bugs found and fixed on the way: `ValueSnapshot` needed an explicit `Identifiable` (Xcode's macOS build failed in `ReadingHistoryView`); `WiggleRoomDeepLink.url(forTrackerId:)` made `nonisolated` (Swift 6 warning in `OpenTrackerIntent`); widget card labels truncated in Fraunces (fixed with `minimumScaleFactor`); section headers and the watch/Mac headings that weren't Fraunces.

**Verified** with real builds (iOS, macOS, watchOS, widgets, complication) and in simulators: iPhone light/dark list, detail, completed/celebration, add-tracker colour picker, settings, Tracker Order, source picker, widget gallery (small/medium/large/chart/All Trackers), watch list and detail. macOS was run on the Mac against real data (list/detail/sidebar). **Not verified:** iPad (no iOS 27 iPad simulator exists here), Live Activity/Dynamic Island, Control Center tile, Lock Screen accessory widgets, complication on a watch face, unit tests (a test run hung and was cancelled). A widget gallery quirk: after reinstalling in a simulator, widget previews can show blank/"Loading data…" until chronod's backoff clears — a simulator artefact, not an app fault.
...
## 2026-09-20 Four more tracker colours

Requested: a darker green, grey, yellow and red. Added Forest, Slate, Sunshine and Cherry as indices 8–11 in [TrackerPalette.swift](../src/WiggleRoomShared/TrackerPalette.swift), each with light/dark values. Appended, not inserted, so existing trackers keep their colour. The picker grid is 4 columns, so 12 swatches lay out 3×4 with no code change. Build-spec §3 updated. Note the first eight were deliberately status-hue-free; the new green/yellow/red are intentionally deeper/softer than the ring traffic-light colours but can still resemble them. Not built or run — no xcodebuild/simulator used this session.

## 2026-09-20 Tracker budget wording; Add Tracker footers removed

"Total budget" is now "Tracker budget" everywhere it appears ([AddTrackerView.swift](../src/WiggleRoom/Views/AddTrackerView.swift), [TrackerDetailView.swift](../src/WiggleRoom/Views/TrackerDetailView.swift), [WatchTrackerDetailView.swift](../src/WiggleRoomWatch/WatchTrackerDetailView.swift)), to make clear it is the amount the tracker is for (e.g. miles or £), not the balance at the end. Removed the explanatory footers under Budget, Source and Reminder on Add Tracker (and the now-unused `budgetFooter`). Kept the "can't be changed after creation" footer on the edit-mode Source section and the Account footer. Not built or run.

## 2026-09-20 Direction explanation on Add Tracker

The Details footer on [AddTrackerView.swift](../src/WiggleRoom/Views/AddTrackerView.swift) now explains the selected direction (`directionFooter`): Decreasing = start with an amount and use it up; Increasing = a reading climbs towards a limit (mileage example). It replaces the old "choose the unit" line and changes with the segmented picker. Not built or run.

## 2026-09-20 Account line restored in iOS tracker header

Connected-source (e.g. Starling) trackers had lost their "connection · account" line on iOS: it was appended to `navigationSubtitle` with a newline, which iOS renders as one line. In [TrackerDetailView.swift](../src/WiggleRoom/Views/TrackerDetailView.swift) the iOS subtitle is now just the date range and the account line is a pinned strip (`safeAreaInset(edge: .top)`, `.bar` background) directly beneath the navigation bar — static while content scrolls, and content is inset below it, not overlaid. macOS keeps its in-content header unchanged. Not built or run, so the visual result on device is unverified.

## 2026-09-20 "Update balance" vs "Refresh" wording

Terminology rule: **Update balance** = the current balance number changes (logged manually or fetched from a source); **Refresh** = the budget figures re-evaluated for the current time, balance untouched. Applied in [TrackerDetailView.swift](../src/WiggleRoom/Views/TrackerDetailView.swift): the pull hint always reads "Pull down to update current balance" (was "refresh" for connected trackers), the macOS button is "Update Current Balance" for all trackers, and the error lines read "Balance update paused…" / "Couldn't update balance…". Watch's connected-source button is now "Update". Not built or run.

## 2026-09-20 Copy pass across the app

Rewrote explanatory copy app-wide to be lighter and plainer, in the calm tone of build-spec §3.6: shorter sentences, no "permanently/cannot", no jargon ("scopes", "rate-limiting", "glyph", "stored token"), consistent "update" wording. Touched: delete confirmations (tracker, delete-all), tracker order footer, appearance footer, connected-source empty states (Tesla mention dropped — Tesla is out of scope), source removal message and Starling token/rate-limit/error lines, Add Tracker hints (period, direction, starting value, budget, source/account, remainder lines — also in `Tracker.swift`, shared with watch/widgets), Log Reading hints, macOS empty state and menu-bar setting, history empty state, "No readings yet" (iOS + watch), detail-screen error lines, and Shortcuts error messages. Wording only; no logic changed. Not built or run, and Dynamic Type/VoiceOver were not re-audited on device.

## 2026-09-20 Chart key shows Actual as green and red

The "Actual" entry in [TrendChartView.swift](../src/WiggleRoomShared/TrendChartView.swift)'s legend showed only green, but the line is green or red per segment depending on pace. `legendItem` now takes a colour list and splits the swatch; Actual shows green and red side by side, the others stay single-colour. Not built or run.

...
- **Menu bar item** (`MenuBarExtra`): compact ahead/behind figure, color-coded per §3.2, with states for refreshing (spinner) and error (`--` with warning color) — see §8.4. Dropdown shows current value, target, difference, and a refresh/log-reading button. **Decision made**: shows a single tracker rather than a submenu of full dashboards; simpler of the two options the spec left open, revisit with a richer submenu if a single pinned tracker proves insufficient once there are enough concurrent trackers for it to matter. **User-configurable**: defaults to the most-recently-started tracker, but a "Show in Menu Bar" submenu in the dropdown (shown once there's more than one tracker) lets the user pin a specific tracker instead, stored as a plain `@AppStorage` preference (not synced data) so it's a per-Mac choice; if the pinned tracker is later deleted, the menu bar silently falls back to the most-recently-started one again.

...
- A **Settings → Connected Sources** screen lists all configured sources, independent of the Tracker list. **Add is implemented** (Starling only so far, §5.3); **remove is not** — a connected source can't currently be deleted/disconnected from this screen once added.
...
- Token stored in the iOS Keychain (`KeychainTokenStore`) with `kSecAttrSynchronizable` set so it syncs via iCloud Keychain across the user's own devices — never stored in UserDefaults or plain files. Abstracted behind a `SecureTokenStore` protocol (`InMemoryTokenStore` for previews/tests) since the real Keychain isn't reachable from a plain XCTest run.
...
- **"Starting value" auto-fills from the live balance**: once the user picks an account in `AddTrackerView`'s target picker, `startingValueText` is overwritten with that account's current Starling balance (`TrackerStore.fetchCurrentValue(for:from:)`) rather than left for the user to type blind — a "Fetching live balance…" indicator shows in place of the usual hint text while it's in flight. Only "Starting value," not "Total budget" — the two stay independent decisions. Silent failure (user can still type by hand).
- **Known gaps, not built this pass**: no way to disconnect/remove a Starling source from Settings → Connected Sources (only add is implemented); Spaces/savings-goal targets; cross-tracker request de-duplication (mitigation 2 above); a watch-side error state for a failed refresh.
- **Findings from real-device testing, not Starling-specific** (see progress-notes.md's "Starling real-device testing" entry for the full diagnosis) — presented to the user as design questions rather than silently patched:
  ...
  2. Any tracker over 7 days defaults its dashboard to the "This Week" zoom (`TrackerDetailView.init`), silently scoping `Target Right Now`/`Under Budget`/"left in budget" to a calendar-week-clamped **fraction** of the real total budget, with no on-screen indication the figures are scoped. "Overall" shows the true total.
  3. **Not a bug**: a brand-new tracker shows a nonzero "Under/Over Budget" figure the instant any time elapses, even with zero real spending — this is the app's intentional linear-pace model (`TrackerPace.computePace`: `difference = expectedConsumedByNow - consumedSoFar`), true for every tracker, always has been.
...
- **Nothing in this Starling implementation has been built or run against a real compiler** — this environment had no Swift toolchain at all (not even `swiftc`, let alone `xcodebuild`/a simulator), so every file described in §5.3 and progress-notes.md's Starling-integration entry is unverified beyond careful manual re-reading. Build in Xcode, resolve any compile errors, and run the test suite (including the new `StarlingAPIClientTests`/`StarlingProviderTests`/`StarlingRequestBudgetTests`/`SecureTokenStoreTests`) before trusting any of it further.
...
- No way to disconnect/remove a Starling source from Settings → Connected Sources yet — only adding one is built.

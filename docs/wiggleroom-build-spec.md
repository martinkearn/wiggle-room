...
- **Refresh cadence (decided)**: while the app is open/foreground, poll every **30 seconds** — this reuses the app's existing `AutoUpdateTicker`/`TrackerUpdateScheduling` scheduling mechanism (currently a 60s display-recompute tick, no network fetch yet), retuned to a flat 30s interval **for every tracker/source, not just Starling**, and extended to actually trigger a data fetch on tick rather than only a display recompute. Manual refresh reuses the existing pull-to-refresh gesture (`.refreshable` on iOS/iPadOS, the equivalent button on macOS) already wired into `handleUpdateGesture()` — that gesture currently no-ops for auto-fetch sources and needs its Starling-fetch branch implemented. In the background, refresh every **5 minutes** via `BGAppRefreshTask` (not yet configured — no `BGTaskScheduler` usage, background-fetch `Info.plist`/entitlement, or background HTTP path exists in the repo as of this decision; needs to be added from scratch). On failure, show a distinct error state (§8.4) rather than a stale/misleading figure.
  - **Risk carried forward, not resolved by this decision**: §12 already flags Starling rate limits as unknown. A 30s foreground poll is far more aggressive than the "not continuous polling" guidance this line used to say, so the real rate limit must be confirmed against current Starling API docs before shipping this cadence — if it can't sustain 30s (especially across multiple trackers sharing one source per §4.4's shared-fetch requirement), fall back to a coarser interval or add client-side coalescing/backoff.
...
- Rate limit handling for Starling API calls (back off gracefully on 429s), especially given §4.4's shared-fetch requirement across trackers on the same source, and now directly relevant given the 30s foreground poll cadence decided in §5.3 — confirm Starling's actual rate limit before building, not after.
...
- **Starling refresh cadence**: 30s foreground poll (reusing/retuning the existing `AutoUpdateTicker`/`TrackerUpdateScheduling` mechanism, applied to all trackers, not just Starling), manual refresh via the existing pull-to-refresh gesture, 5-minute background refresh via `BGAppRefreshTask`. See §5.3. Implementation itself (Keychain wrapper, HTTP client, `BGTaskScheduler` setup) not yet built — see §12's open items.
